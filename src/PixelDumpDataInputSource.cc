...
#include <memory>
#include "PixelDumpDataInputSource.h"
...
int PixelDumpDataInputSource::getEventNumberFromFillWords(const std::vector<uint64_t>& buffer, uint32_t & totword ){
...
// constructor
PixelDumpDataInputSource::PixelDumpDataInputSource(const edm::ParameterSet& pset, 
  ...
  m_eventnumber_shift(0)
{
  ...
  else               std::cout<<" Will keep the original run number "<<std::endl;
  ...
  if ( !is_header(m_data) ) {  // check that it is not a header 
    ...
    if(m_runnumber != -1) {
      std::cout<<" Force run number to "<<m_runnumber<<" was "<<runnum<<std::endl;
      edm::LogInfo("") << "Overwriting run number as defined in .cfg file!!! Run number now set to " 
		       << m_runnumber << " (was " << runnum << ")";
    } else {
      m_runnumber=runnum;
    }
  } 
...
// destructor
PixelDumpDataInputSource::~PixelDumpDataInputSource() {


}

bool PixelDumpDataInputSource::is_header(unsigned long long value) {
...
bool PixelDumpDataInputSource:: is_trailer(unsigned long long value) {
  // replace to make it work for uTCA FEDs with 48 channels
...
bool PixelDumpDataInputSource::setRunAndEventInfo(edm::EventID& id, edm::TimeValue_t& time, edm::EventAuxiliary::ExperimentType&) {
  ...
  do {
    ...
    edm::LogInfo("PixelDumpDataInputSource::produce()") << "**** event number = " << eventnumber << " global event number " << m_currenteventnumber << " data " << std::hex << m_data << std::dec << std::endl;
...
void PixelDumpDataInputSource::produce(edm::Event& event) {
...
uint32_t PixelDumpDataInputSource::synchronizeEvents(){
  int32_t result= m_currenteventnumber -1;
      
  return(uint32_t) result;
}
//DEFINE_FWK_MODULE(PixelDumpDataInputSource);
